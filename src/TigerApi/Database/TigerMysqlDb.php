...
class TigerMysqlDb implements ICanGetDbConnection {
  ...
  public function __construct(IAmMySqlConnectionCredentials $connectionCredentials) {
    ...
    $this->db = new Connection('mysql:host='.$connectionCredentials->getHost().';dbname='.$connectionCredentials->getDbName(), $connectionCredentials->getUserName(), $connectionCredentials->getPassword(),['lazy' => true, Mysql::ATTR_FOUND_ROWS => true]);
