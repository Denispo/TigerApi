...
class TigerMysqlDb implements ICanGetDbConnection {
  ...
  public function __construct(IAmMySqlConnectionCredentials $connectionCredentials) {
    // \PDO::MYSQL_ATTR_FOUND_ROWS => TRUE means: UPDATE command will return affectedRows = 1 even if new and current value has the same value ie. nothing is actually updated.
