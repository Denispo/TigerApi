...
use Nette\Database\Drivers\PDO\MySQL\Driver;
use Pdo\Mysql;
...
class TigerMysqlDb implements ICanGetDbConnection {
  ...
  public function __construct(IAmMySqlConnectionCredentials $connectionCredentials) {
    // Mysql::ATTR_FOUND_ROWS => TRUE means: UPDATE command will return affectedRows = 1 even if new and current value has the same value i.e. nothing is actually updated.
    ...
    // This behavior cause unability to deretmine if record in DV was not found, or was found but contains already contains new value
     ...
     $driver = new Driver(
        dsn: 'mysql:host='.$connectionCredentials->getHost().';dbname='.$connectionCredentials->getDbName(),
        username: $connectionCredentials->getUserName(),
        password: $connectionCredentials->getPassword(),
        options: ['lazy' => true, Mysql::ATTR_FOUND_ROWS => true]
     );
